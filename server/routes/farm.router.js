...
router.post("/", rejectUnauthenticated, (req, res) => {
  ...
  const farmValues = [
    req.body.payload.farm_name,
    req.body.payload.street_address,
    req.body.payload.city,
    req.body.payload.state,
    req.body.payload.zipcode,
    req.body.payload.phone,
    req.body.payload.size,
    req.body.payload.type,
    req.body.payload.bio,
    req.user.id,
    req.body.latitude,
    req.body.longitude,
  ];
  pool
    .query(farmQueryText, farmValues)
    .then((response) => {
      res.sendStatus(201);
    })
    .catch((error) => {
